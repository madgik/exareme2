...
class TestUDFGen_TransferInputandReturnType(TestUDFGenBase):
    ...
    def udfregistry(self):
        ...
        @udf(
            t=literal(),
            prev_state=transfer_object(DummyTransferClass),
...
LANGUAGE PYTHON
{
    ...
    class DummyTransferClass(BaseModel):
        num: int
        list_of_nums: List[int]
    state_str = _conn.execute("SELECT jsonified_object from prev_state_table;")['jsonified_object'][0]
    state = DummyTransferClass.parse_raw(state_str)
    t = 5
    result = DummyTransferClass(nums=t, list_of_nums=[t, t, t])
    return result.json()
}"""
...
FROM
    ...
    def test_generate_udf_queries(
        ...
        udfdef, udfsel = generate_udf_queries(funcname, positional_args, {})
        raise ValueError(udfdef.template)
        assert udfdef.template == expected_udfdef
        assert udfsel.template == expected_udfsel
