...
def context_id(request_id):
    ...
    clean_up_node1.delay(request_id=request_id, context_id=context_id.lower()).get()
    clean_up_node2.delay(request_id=request_id, context_id=context_id.lower()).get()
    clean_up_global.delay(request_id=request_id, context_id=context_id.lower()).get()


def test_create_merge_table_with_remote_tables(request_id, context_id):
    ...
    local_node_1_insert_data_to_table.delay(
        request_id=request_id, table_name=local_node_1_table_name, values=values
    ).get()
    local_node_2_insert_data_to_table.delay(
        request_id=request_id, table_name=local_node_2_table_name, values=values
    ).get()
    ...
    merge_tables = global_node_get_merge_tables.delay(
        request_id=request_id, context_id=context_id
    ).get()
    ...
    table_data_json = global_node_get_merge_table_data.delay(
        request_id=request_id, table_name=merge_table_name
    ).get()
