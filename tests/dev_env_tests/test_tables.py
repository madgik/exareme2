...
local_node_get_tabular_data = get_celery_task_signature(local_node, "get_tabular_data")
...
def test_create_and_find_tables(context_id):
    ...
    tabular_data_json = local_node_get_tabular_data.delay(table_name=table_1_name).get()
    ...
    assert tabular_data.name == table_1_name
    assert tabular_data.columns == expected_columns
    ...
    tabular_data_json = local_node_get_tabular_data.delay(table_name=table_2_name).get()
    tabular_data = TabularData.parse_raw(tabular_data_json)
    expected_columns = [
        ColumnDataInt(name="col1", data=[1, 2, 3]),
    ...
    assert tabular_data.name == table_2_name
    assert tabular_data.columns == expected_columns
