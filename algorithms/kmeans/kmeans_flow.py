...
class AlgorithmFlow(AlgorithmFlow_abstract):
    ...
    async def run(self):
        ...
        udf_result_schema=[ f"v{i} DOUBLE" for i in range(num_of_dimensions)]
        udf_result_schema=",".join(udf_result_schema) #-->"centroid_d1 DOUBLE, centroid_d2 DOUBLE, ..."

        centroids_table_name=await runtime_interface.execute_udf_on_global_node("generate_initial_centroids",udf_result_schema,[num_of_dimensions,num_of_clusters,initial_min,initial_max])
        runtime_interface.create_remote_global_to_locals(centroids_table_name)

        for i in range(num_of_iterations):
            local_centroids=await runtime_interface.execute_udf_on_all_local_nodes("local_calc",result_table_schema,[self.data_table_name,centroids_table_name])

            merged_table_name=await runtime_interface.create_remote_and_merge_on_global_node(local_centroids)

            centroids_table_name=await  runtime_interface.execute_udf_on_global_node("global_calc",num_of_clusters,merged_table_name)
            await  runtime_interface.create_remote_global_to_locals(table,centroids_table_name)
...
##async def init_dbs():
##    await runtime_interface.init_dbs()
...
####        udf_result_size=num_of_dimensions
