...
def var(*args):
    ...
    if len(args) == 1:
        if hasattr(functions.variables, var):
            return functions.variables.__dict__[var].encode()
        else:
            raise functions.OperatorError(
                "var", "Variable '" + var + "' does not exist"
            )
    elif len(args) == 2:
        functions.variables.__dict__[var] = args[1].decode()
        return functions.variables.__dict__[var].encode()
    else:
        return None
...
def requirevars(*args):
    ...
    for v in (" ".join(args).strip()).split():
        if not hasattr(functions.variables, v):
            raise functions.OperatorError(
                "requirevars", "Variable %s isn't initialized" % v
            )
    return 1
...
def setexecdb(*args):
    ...
    if len(args) == 0 and hasattr(functions.variables, var):
        ...
        functions.variables.__dict__[var] = str(
            os.path.abspath(
                os.path.expandvars(os.path.expanduser(os.path.normcase(args[0])))
            )
        )
        return str(functions.variables.__dict__[var])
