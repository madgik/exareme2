...
def htmlencode(*args):
    ...
    return xml.sax.saxutils.escape("".join(args[0]), {'"': "&quot;"})
...
def htmladdbreaks(*args):
    ...
    out = "".join([unicode(x) for x in args])
...
def htmllink(*args):
    ...
    return (
        ...
        + addhttp(args[0])
        + '">'
        + htmladdbreaks(htmlencode(unicode(args[0])))
        + "</a>"
    )
