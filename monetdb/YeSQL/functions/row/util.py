...
def reloadmod():
    for module_name in list(sys.modules.keys()):
        # Exclude some built-in modules and others that should not be reloaded
        if (
            module_name.startswith("row.")
            or module_name.startswith("aggregate.")
            or module_name.startswith("vtable.")
            or module_name.startswith("udfs.")
        ):
...
def exec(cursor, query):
    result = cursor.execute("select * from mytable((" + query + "))")
    results = cursor.fetchall()
    for row in results:
        return functions.results[row[0]]


def gcfree(arg=None) -> int:
    gc.collect()
    gc.collect()
    gc.collect()
    gc.collect()
...
gcfree.registered = True


def set_reload(arg) -> int:
    functions.RELOAD = arg
    if arg == 1:
        # pass
        reloadmod()
    return functions.RELOAD


set_reload.registered = True
...
get_debug.registered = True


def set_debug(arg=None) -> int:
    if arg == 1:
        functions.DEBUG = True
    elif arg == 0:
        functions.DEBUG = False
    elif arg == None:
        if functions.DEBUG == True:
            functions.DEBUG = False
        elif functions.DEBUG == False:
            functions.DEBUG = True
    return functions.DEBUG


set_debug.registered = True
...
def urlrequest(*args):
    ...
    try:
        req = urllib.request.Request(
            "".join((x.encode("utf-8") for x in args if x != None)),
            None,
            domainExtraHeaders,
        )
...
def urlrequestpost(*args):
    ...
    try:
        ...
        hreq = urllib.request.urlopen(req, urllib.parse.urlencode(dataout))
...
def execprogram(*args):
    ...
    if args[1] == None:
        if raise_error:
            ...
            raise functions.OperatorError(
                "execprogram",
                "Second parameter should be the name of the program to run",
            )

    outtext = errtext = ""
    try:
        p = subprocess.Popen(
            [str(x) for x in args[1:] if x != None],
            stdin=subprocess.PIPE if args[0] != None else None,
    ...
    if p.returncode != 0:
        if raise_error:
            ...
            raise functions.OperatorError(
                "execprogram", functions.mstr(errtext).strip()
            )

    try:
        outtext = str(outtext, "utf-8")
    except KeyboardInterrupt:
        raise
    except:
