...
strip_remove_newlines = re.compile(
    "(?:\\s+$|^\\s+|(?<=[^\\s\\d\\w.;,!?])\n+)", re.UNICODE
)
...
match_field_all = re.compile(
    "(title|isbn|issn|subject|creator|language|type)\sall", re.UNICODE
)
...
query_regular_characters = re.compile(
    r"""^[·∆©(́−·¨¬…‐"•΄€„”“‘’´«»’ʹ–\w\s\[!-~\]]*$""", re.UNICODE
)
...
def regexprfindall(*args):
    ...
    if len(args) != 2:
        raise functions.OperatorError(
            "regexprfindall", "Two parameters should be provided"
        )
...
def regexprmatches(*args):
    ...
    if len(args) != 2:
        raise functions.OperatorError(
            "regexprmatches", "Two parameters should be provided"
        )
...
def regexpcountwords(*args):
    ...
    return sum(
        (
            (i.group().strip().count(" ") + 1)
            for i in re.finditer(args[0], str(args[1]), re.UNICODE)
        )
    )
...
def contains_str(*args):
    ...
    if len(args) != 2:
        raise functions.OperatorError(
            "included", "operator takes exactly two arguments"
        )
    if args[1] in args[0]:
        return True
    return False
...
def stripchars(*args):
    ...
    if len(args) < 1:
        raise functions.OperatorError(
            "stripchars", "operator takes at least one arguments"
        )
    if args[0] == None:
        return None
    if len(args) < 2:
...
def hashmd5mod(*args):
    ...
    if len(args) == 2:
        ...
        return (
            int(hashlib.md5(chr(30).join([repr(x) for x in args])).hexdigest(), 16)
            % args[-1]
        )
...
def textwindow(*args):
    ...
    yield tuple(
        itertools.chain(
            ("prev" + str(x) for x in range(1, abs(prev) + 1)),
            ("middle",),
            ("next" + str(y) for y in range(1, nextlen + 1)),
        )
    )
    ...
    if prev >= 0:
            ...
            if pattern == None:
                for i in range(len(g) - window + 1):
                    yield (
                        g[i : i + prev]
                        + [" ".join(g[i + prev : i + pm])]
                        + g[i + prev + middle : i + window]
                    )
            else:
                patt = re.compile(pattern, re.UNICODE)
                for i in range(len(g) - window + 1):
                    mid = b" ".join(g[i + prev : i + pm])
                    if patt.search(mid):
        ...
        if middle == 1:
            ...
            for i in range(len(g) - window + 1):
                ...
                if patt.search(g[i]):
                    yield tuple(
                        itertools.chain(winprev, ([mid] + g[i + middle : i + window]))
                    )
                else:
                    winprev.append(g[i])
...
def contains_six_digit_number(input_string):
    # Split the input string into words
    words = input_string.split()

    # Iterate through the words and check if any is a 6-digit number
    for word in words:
        if len(word) == 6 and word.isdigit():
            return True

    # If no 6-digit number is found, return False
    return False


# patt = re.compile(r'\b\d{6}\b')
def textwindow2s(*args) -> Generator[str, str, str]:
    """
    ...
    try:
        ...
        try:
            ...
            raise functions.OperatorError(
                "textwindow2s", "Third argument should be an integer"
            )
    except IndexError:
        nextlen = 0
    ...
    if len(args) > 4:
        try:
            # pass
            patt = re.compile(args[4].decode())
        except:
            raise functions.OperatorError(
                "textwindow2s", "Fourth argument must be string or compiled pattern"
            )
        for i in range(len(g) - middle + 1):
            ...
            if patt.search(mid):
                yield " ".join(
                    g[max(i - prev, 0) : i]
                ).encode(), mid.encode(), " ".join(g[im : im + nextlen]).encode()
    else:
        for i in range(len(g) - middle + 1):
            im = i + middle
            # print(' '.join(g[max(i-prev,0):i]).encode(),' '.join(g[i:im]).encode(), ' '.join(g[im:im+nextlen]).encode())
            yield " ".join(g[max(i - prev, 0) : i]).encode(), " ".join(
                g[i:im]
            ).encode(), " ".join(g[im : im + nextlen]).encode()
