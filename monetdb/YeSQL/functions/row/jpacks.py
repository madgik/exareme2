...
def jzip(*args):
    ...
    """
    return json.dumps(
        [list(x) for x in zip(*jopts.elemfromj(*args))],
        separators=(",", ":"),
        ensure_ascii=False,
    )
...
def jzipdict(*args):
    ...
    """
    return json.dumps(
        dict(tuple([x[0], x[1:]]) for x in zip(*jopts.elemfromj(*args))),
        separators=(",", ":"),
        ensure_ascii=False,
    )
...
def jlen(*args):
    ...
    """
    return sum(
        [len(x) if type(x) in (dict, list) else 1 for x in (jopts.elemfromj(*args))]
    )
...
def jintersection(*args):
    ...
    if len(args) < 2:
        raise functions.OperatorError(
            "jintersection", "operator needs at least two inputs"
        )
...
def jmergeregexp(*args):
    ...
    if len(inp) > 0 and type(inp[0]) == list:
        ...
        res = "|".join(
            "(?P<" + x + ">" + "|".join(y) + ")" for x, y in out.items() if y != ""
        )
        if res == "":
            res = "_^"
        return res
            ...
            try:
                if i[0] == "{" and i[-1] == "}":
                    keys.update(
                        [
                            (x, None)
                            for x in json.loads(i, object_pairs_hook=OrderedDict).keys()
                        ]
                    )
            except TypeError as e:
                pass
...
def jdictgroupkey(*args):
    ...
    def recgroupkey(jdict, gkeys):
        ...
        if len(gkeys) > 1:
            outdict = OrderedDict(
                [(x, recgroupkey(y, gkeys[1:])) for x, y in outdict.items()]
            )
