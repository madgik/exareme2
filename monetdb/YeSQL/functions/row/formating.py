...
def strsplit(*args):  ###splits the first arguments
    ...
    args = args[1:]
    try:
        largs, kargs = argsparse.parse(
            args, csvargs.boolargs, csvargs.nonstringargs, csvargs.needsescape
        )
    except Exception as e:
        raise functions.YeSQLError(e)
    ...
    if len(largs) > 0:
        raise functions.OperatorError(
            "strsplit", "strsplit operator: Unknown argument %s" % ("".join(largs))
        )
    f = io.StringIO()
...
def tokenize(*args):  # ->Iterator[str]: ###splits the first arguments
    """
    ...
    if len(largs) > 0:
        raise functions.OperatorError(
            "strsplitv", "strsplit operator: Unknown argument %s" % ("".join(largs))
        )
    f = io.StringIO()
...
def strjoin(*args):
    ...
    if len(largs) > 0:
        raise functions.OperatorError(
            "strjoin", "Unknown argument %s" % ("".join(largs))
        )
    if "dialect" not in kargs:
        kargs["dialect"] = csvargs.defaultcsv()
    f = io.StringIO()
