...
class jgroup:
    ...
    def step(self, *args):
        if len(args) == 1:
            self.outgroup += jopts.elemfromj(args[0].decode())
        else:
            self.outgroup.append(jopts.elemfromj(*args))

    def final(self):
        return jopts.toj(self.outgroup).encode()
    ...
    def step(self, *args):
        for arg in args:
            ...
            if not set(v).issubset(self.outgroup):
                ...
                self.outgroup.update(
                    [
                        (k, None)
                        for k in json.loads(arg, object_pairs_hook=OrderedDict).keys()
                    ]
                )
