...
class freqitemsets:
                ...
                if v[0] == "threshold":
                    try:
                        ...
                        raise functions.OperatorError(
                            "FreqItemsets", "No integer value given for threshold"
                        )
                if v[0] == "noautothres":
                    self.autothres = 0
                if v[0] == "compress":
    ...
    def final(self):
        ...
        if not self.stats:
            for its, v in sorted(
                list(splist[1].items()), key=itemgetter(1), reverse=True
            ):
                self.itemset_id += 1
                for i in self.demultiplex(
                    (
                        self.itemset_id,
                        len([self.codekw[i] for i in its]),
                        v,
                        [self.codekw[i] for i in its],
                    )
                ):
                    yield i
        ...
        for l in range(2, min(self.maxlength + 1, self.maxlen + 1)):
            ...
            if self.autothres == 1:
                if len(self.input) == 0 or len(self.passedkw) == 0:
                    ...
                    self.threshold = self.startingthreshold + int(
                        len(self.passedkw) / len(self.input)
                    )
            ...
            if not self.stats:
                for its, v in sorted(
                    list(splist[l].items()), key=itemgetter(1), reverse=True
                ):
                    self.itemset_id += 1
                    for i in self.demultiplex(
                        (
