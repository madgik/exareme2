...
class mymedian:
    ...
    def final(self):
        ...
        if self.counter % 2:  # Number of elements in data set is even.
            self.median = self.sample[int(self.counter / 2)]
        else:  # Number of elements in data set is odd.
            midpt = int(self.counter / 2)
...
class variance:
        ...
        if not args:
            ...
            if tmp == "false" or tmp == "sample":
                ...
                raise functions.OperatorError(
                    "sdev",
                    "Wrong value in second argument" + "\n" + "Accepted Values:" + "\n"
    ...
    def final(self):
        ...
        try:
            if (
                not self.population and self.n > 1
    ...
    def initargs(self, args):
        ...
        if not args:
            ...
            if self.p > 2 or self.p < -1:
                raise functions.OperatorError(
                    "\n gmean",
                    "Second argument takes values from -1 to 2\n"
    ...
    def step(self, *args):
        ...
        if not (isinstance(args[0], str)) and args[0]:
            if self.p < 1 and args[0] < 1:
                raise functions.OperatorError(
                    "gmean",
                    "The specified type of mean applies only to positive numbers",
                )
...
class pearson:
    ...
    def final(self):
        ...
        d = math.sqrt(self.n * self.sX2 - self.sX * self.sX) * math.sqrt(
            self.n * self.sY2 - self.sY * self.sY
        )
