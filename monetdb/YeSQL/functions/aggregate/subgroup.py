...
def timedelta2millisec(tdelta):
    return (
        tdelta.days * 24 * 60 * 60 * 1000 + tdelta.seconds * 1000 + tdelta.microseconds
    )
...
class datediffbreak:
            ...
            if args[i] == "order":
                ...
                if len(args) < 5:
                    raise functions.OperatorError(
                        "datediffbreak", "Wrong number of arguments"
                    )
                break
    ...
    def step(self, *args):
        ...
        if not self.position:
            ...
            self.vals.append(
                list(args[: self.position - 1] + args[self.position + 1 :])
            )
...
class datediffnewsesid:
    ...
    def initargs(self, args):
        ...
        if len(args) < 4:
            raise functions.OperatorError(
                "datediffnewsesid", "Wrong number of arguments"
            )
        self.maxdiff = args[0]
    ...
    def final(self):
        ...
        yield tuple(
            ["bgroupid"] + ["C" + str(i) for i in range(1, len(self.vals[0]) - 1)]
        )
    ...
    def final(self):
        if self.tablesize <= 0:
            ...
            yield tuple(
                ["groupid"]
                + ["date"]
                + ["C" + str(i + 1) for i in range(self.tablesize - 1)]
            )
