...
class graphpowerhash:
    ...
    def final(self):
                ...
                int(
                    min(
                        ...
                        (
                            (3 * invdegree + 3)
                            * math.log(ncount)
                            / math.log(math.log(ncount))
                            if ncount > 16
                            else ncount
                        ),
                        # Simon Mukwembi "A note on diameter and the degree sequence of a graph"
                        1 + 3 * (ncount - len(degreeseq) + 1) / float((mindegree + 1)),
                        ncount - len(degreeseq) + 2,
                    )
                )
                / 2
            )
            ...
            for s in range(self.steps):
                ...
                for n, v in self.nodes.items():
                    nhash = md5(
                        ...
                        + chr(30)
                        + chr(30).join(
                            sorted([nhashes[x] + chr(29) + y for x, y in v[0]])
                        )
                    ).digest()
                    nhashes1[n] = nhash
        ...
        return json.dumps(
            [b2a_base64(x)[0:-3] for x in sorted(nhashes.values())],
            separators=(",", ":"),
            ensure_ascii=False,
        )
        ...
        for n, v in self.nodes.items():
            for e in v[0]:
                tgf += (
                    ...
                    + clearname(e[0])
                    + " "
                    + (clearname(e[1]) if e[1] != None else "")
                    + "\n"
                )
...
class graphcliques:
    ...
    def step(self, *args):
        if len(args) != 2:
            raise functions.OperatorError(
                "graphcliques", "Two parameters should be provided"
            )
    ...
    def _bors_kerbosch(self, R, P, X):
        ...
        for v in P.difference(self.G[pivot]):
            for c in self._bors_kerbosch(
                R.union((v,)), P.intersection(self.G[v]), X.intersection(self.G[v])
            ):
                yield c
            P.remove(v)
            X.add(v)
