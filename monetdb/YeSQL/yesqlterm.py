...
def update_tablelist():
    ...
    # monetdb specific - get list of tables
...
def schemaprint(cols):
    ...
    if cols != []:
        ...
        if colschars <= 80:
            ...
            for i in cols:
                charspercolname = max(
                    (totalchars - colschars) / (len(cols) + 1 - i1) - 5, mincolchars
                )
...
def printrow(row):
    ...
    for d in row:
        ...
        if type(d) in (int, float, bool, Decimal):
            d = str(d)
...
def setConnection(connection, functions):
    try:
...
def process_args():
    global connection, errorexit, db, nobuf, externalpath, ENGINE

    import argparse
    import sys

    parser = argparse.ArgumentParser()
    # monetdb specific command line args
    parser.add_argument("-d", help="database file name")
    parser.add_argument("-f", help="absolute path with udfs (it's a directory)")
    parser.add_argument("-P", help="db port")
    parser.add_argument("-H", help="db host")
    parser.add_argument("-u", help="db username")
    parser.add_argument("-p", help="db password")
    parser.add_argument("-e", help="db engine")
    ...
    if args.f is not None:
        ...
        externalpath = None
    functions.DIALECT = args.e
    functions.imports(args.e)
    functions.sqltransform.setimports(args.e)
    Connection = functions.Connection()
    connection = functions.dbdialect.createConnection(
        Connection, user, password, host, db, port
    )
    # connection = Connection(*functions.dbdialect.connection_string( user, password, host, db, port)) # for sqlite
    connection = setConnection(connection, functions)
    if db == "" or db == ":memory":
        pass
    else:
        pass
...
VERSION = "1.0"
mtermdetails = "YeSQL - version " + VERSION
...
if not pipedinput:
    print(mtermdetails)
    print(
        "running on Python: " + ".".join([str(x) for x in sys.version_info[0:3]]),
        end=" ",
    )
    try:
        sys.stdout.write(", YeSQL: " + functions.VERSION + " , " + ENGINE + "\n")
    except:
        print()
    print(intromessage)
...
while True:
    ...
    if iscommand:
        ...
        # monetdb specific
            ...
            try:
                statement = re.match(r"\s*\.queryplan\s+(.+)", origstatement).groups()[
                    0
                ]
                queryplan = True
            except IndexError:
            ...
            if not argument:
                ...
                if argument not in alltables:
                    ...
                    if "." in argument:
                        ...
                        argument = "".join(sa[1:])
                    statement = (
                        "select sql from (select * from "
                        + db
                        + ".sqlite_master union all select * from sqlite_temp_master) where tbl_name like '%s' and sql is not null;"
                        % (argument)
                    )
            ...
            if not argument:
                ...
                for i in functions.functions:
                    if argument in functions.functions[i]:
                        ...
                        printterm(
                            functions.mstr(functions.functions[i][argument].__doc__)
                        )
    ...
    if statement:
        ...
        try:
            ...
            if beeping:
                printterm("\a\a")
            # cursor = connection.cursor()
            # cursor.execute('select gcfree(1);')
            # cursor.close()
            colscompl = []
            updated_tables = set()
            ...
            for i in reversed(
                traceback.format_exc(limit=sys.getrecursionlimit()).splitlines()
            ):
                trlines.append(i)
