...
import sqlparse
import sqlparse.sql
from functions.monetdb_dialect import datatypemap
from functions.monetdb_dialect import monetdatatypes
from functions.monetdb_dialect import parseplan
from functions.monetdb_dialect import register_expand
...
class Transclass:
    ...
    def rectransform(self, con, s, s_orig=None):
        ...
        if (t != None or tmatch2 != None) and t.groups()[
            ...
            if subq != "":
                if len(preposition) > 0:
                    ...
                    paramslist += [format_param_scalar(subq)]
        ...
        for t in flatten_with_type(select_range, sqlparse.sql.Function):
            ...
            if funcname in self.multiset_functions:
                multisetfuncs += 1
                ...
                # monetdb specific parse explain
                ...
                for expandtypes in lines[0]:
                    if expandtypes[0] not in fname_returnx:
                        expanddatatypes.append(monetdatatypes[expandtypes[1]])
                        num += 1
                    else:
                ...
                out_vtables += [
                    (
                        vname,
                        "expand_" + "_".join([x for x in expanddatatypes]),
                        format_query(ss),
                    )
                ]
                register_expand(
