...
// yesql version
typedef char *(*jitted_function)(int paramsnum, int myinputs, int outputsnum, void **inputs, void **outputs,
                                        malloc_function_ptr malloc, free_function_ptr free);
...
#define GENERATE_BAT_INPUT_BASE(tpe)                                           \
	...
	inputs[index] = bat_data;                                                  \
        if (strcmp(#tpe, "bte") == 0)             \
                bat_data->datatype = 4;                           \
        else if (strcmp(#tpe, "sht") == 0)                  \
                bat_data->datatype = 5;                           \
        else if (strcmp(#tpe, "bit") == 0)                  \
                bat_data->datatype = 4;                           \
        else if (strcmp(#tpe, "int") == 0)                 \
                bat_data->datatype = 1;                          \
        else if (strcmp(#tpe, "oid") == 0)            \
                bat_data->datatype = 1;             \
        else if (strcmp(#tpe, "lng") == 0)       \
                bat_data->datatype = 6;              \
        else if (strcmp(#tpe, "flt") == 0)         \
                bat_data->datatype = 2;                \
        else if (strcmp(#tpe, "dbl") == 0)          \
                bat_data->datatype = 2;                  \
        else if (strcmp(#tpe, "str") == 0)         \
                bat_data->datatype = 0;                  \
        else if (strcmp(#tpe, "date") == 0)        \
                bat_data->datatype = 3;                 \
        else if (strcmp(#tpe, "daytime") == 0)         \
                bat_data->datatype = 3;                   \
        else if (strcmp(#tpe, "timestamp") == 0)        \
                bat_data->datatype = 3;              \
        else if (strcmp(#tpe, "blob") == 0)            \
                bat_data->datatype = 0;                 \
        else         \
                bat_data->datatype = 0;    \
...
#define GENERATE_BAT_INPUT(b, tpe)                                             \
        {                                                                          \
                ...
                if (BATtdense(b)) {                                     \
                        ...
                        bat_data->data = wrapped_GDK_malloc(                        \
					...
					bool grouped)
{
	...
	//const char *compilation_flags = debug_build ? "-g -O0" : "-O2";
        // yesql_version
        const char *compilation_flags = debug_build ? "-g -O0" : "-O3 -march=native";
	...
	//msg = func(inputs, outputs, wrapped_GDK_malloc, wrapped_GDK_free);
        //yesql_version
        msg = func(input_count, myinputs, output_count, inputs, outputs, wrapped_GDK_malloc, wrapped_GDK_free);
	...
	for (i = 0; i < (size_t)pci->retc; i++) {
		...
		if (bat_type == TYPE_bit || bat_type == TYPE_bte ||
			...
			if (bat_type == TYPE_date) {
				...
				BATsetcount(b, count);
                                for (j=0;j<count;j++){
                                    int res = tfastins_nocheckVAR(b, j, ((char**)data)[j]);
                                    res+=1;
                                }
				GDKfree(data);
			} else if (bat_type == TYPE_blob) {
