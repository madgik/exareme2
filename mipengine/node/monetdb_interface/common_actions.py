...
def get_table_schema(table_name: str) -> TableSchema:
    ...
    """
    schema = MonetDBPool().execute_and_fetchall(
        f"""
...
def get_table_type(table_name: str) -> TableType:
    ...
    monetdb_table_type_result = MonetDBPool().execute_and_fetchall(
        f"""
        SELECT type
        FROM
        tables
...
def get_table_names(table_type: TableType, context_id: str) -> List[str]:
    ...
    """
    table_names = MonetDBPool().execute_and_fetchall(
        f"""
...
def get_table_data(table_name: str) -> List[ColumnData]:
    ...
    row_stored_data = MonetDBPool().execute_and_fetchall(
        f"""
        SELECT {table_name}.*
        FROM {table_name}
...
def get_data_models() -> List[str]:
    ...
    data_models_code_and_version = MonetDBPool().execute_and_fetchall(
...
def get_dataset_code_per_dataset_label(data_model) -> Dict[str, str]:
    ...
    datasets_rows = MonetDBPool().execute_and_fetchall(
        f"""
        SELECT code, label
...
def get_data_model_cdes(data_model) -> CommonDataElements:
    ...
    cdes_rows = MonetDBPool().execute_and_fetchall(
        f"""
...
def _drop_table_by_type_and_context_id(table_type: TableType, context_id: str):
    ...
    context_id : str
        The id of the experiment
    """
    table_names_and_types = MonetDBPool().execute_and_fetchall(
        f"""
    ...
    for name, table_type in table_names_and_types:
        if table_type == _convert_mip2monet_table_type(TableType.VIEW):
            MonetDBPool().execute(f"DROP VIEW {name}")
        else:
            MonetDBPool().execute(f"DROP TABLE {name}")
...
def _drop_udfs_by_context_id(context_id: str):
    ...
    context_id : str
        The id of the experiment
    """
    function_names = MonetDBPool().execute_and_fetchall(
        f"""
    ...
    for name in function_names:
        MonetDBPool().execute(f"DROP FUNCTION {name[0]}")
