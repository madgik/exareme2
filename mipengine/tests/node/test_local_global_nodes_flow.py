...
global_node_get_merge_table_data = nodes_communication.get_celery_get_merge_table_data_signature(global_node)
...
def test_create_merge_table_with_remote_tables():
    ...
    # Create local tables
    local_node_1_table_name = local_node_1_create_table.delay(context_id,
                                                              str(pymonetdb.uuid.uuid1()).replace("-", ""),
                                                              schema.to_json()
                                                              ).get()
    local_node_2_table_name = local_node_2_create_table.delay(context_id,
                                                              str(pymonetdb.uuid.uuid1()).replace("-", ""),
                                                              schema.to_json()
                                                              ).get()
    ...
    global_node_create_remote_table.delay(table_info_local_1.to_json(), monetdb_url_local_node_1).get()
    global_node_create_remote_table.delay(table_info_local_2.to_json(), monetdb_url_local_node_2).get()
    ...
    # Create merge table
    merge_table_name = global_node_create_merge_table.delay(context_id,
                                                            str(pymonetdb.uuid.uuid1()).replace("-", ""),
                                                            remote_tables
                                                            ).get()
