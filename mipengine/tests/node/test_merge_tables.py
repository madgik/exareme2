...
def setup_db_tables_for_merge(number_of_table: int):
    connection = get_node_db_connection("local_node_1")
...
def test_merge_tables():
    success_partition_tables = [setup_db_tables_for_merge(1), setup_db_tables_for_merge(2), setup_db_tables_for_merge(3),
                                setup_db_tables_for_merge(4)]
...
def test_incompatible_schemas_merge():
    with pytest.raises(IncompatibleSchemasMergeException):
        connection = get_node_db_connection("local_node_1")
        cursor = connection.cursor()
        incompatible_table = setup_db_tables_for_merge(5)
        cursor.execute(f"ALTER TABLE {incompatible_table} DROP {'col1'}")
        connection.commit()

        incompatible_partition_tables = [setup_db_tables_for_merge(1), setup_db_tables_for_merge(2),
                                         setup_db_tables_for_merge(3), setup_db_tables_for_merge(4), incompatible_table]
...
def test_table_cannot_be_found():
    with pytest.raises(TableCannotBeFound):
        not_found_tables = [setup_db_tables_for_merge(1), setup_db_tables_for_merge(2), setup_db_tables_for_merge(3),
                            setup_db_tables_for_merge(4), "non_existant_table"]
