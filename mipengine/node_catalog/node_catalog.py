...
from mipengine.common.node_catalog_DTOs import NodeRecord
...
class NodeRegistry:
    ...
    async def register_node(self, node_record: NodeRecord):
        ...
        self._registry.append(node_record)

    async def deregister_node(self, node_id):
        self._registry = [
            node_record
            for node_record in self._registry
    ...
    class TaskQueueIPAlreadyInUseError(Exception):
        def __init__(self, node_record):
            self.message = (
                f"Node with node_id:{node_record.node_id} is "
                "already registered with task_queue_ip: "
                "{node_record.task_queue_ip}"
            )
    ...
    class DatabaseIPAlreadyInUseError(Exception):
        def __init__(self, node_record):
            self.message = (
                f"Node with node_id:{node_record.node_id} is "
                "already registered with db_ip: {node_record.db_ip}"
            )

    class NodeIdAlreadyInRegistryError(Exception):
        def __init__(self, node_record):
            self.message = (
                f"A node with id:{node_record.node_id} already"
                "exists in the NodeRegistry. Either first deregister the node "
                "and then register it or update enregistered node"
            )
