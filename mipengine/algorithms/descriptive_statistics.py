...
def populate(response, numerical_variables, categorical_variables, enumerations_dict):
    ...
    for i, curr_numerical in enumerate(numerical_variables):
        for j in range(num_datasets):
            numerical_list.append(
                numerical_to_result(curr_numerical, str(j), response, i)
            )
    ...
    for i, curr_categorical in enumerate(categorical_variables):
        for j in range(num_datasets):
            categorical_list.append(
                categorical_to_result(
                    curr_categorical, str(j), response, i, enumerations_dict
                )
            )
        global_result = categorical_to_result(
            curr_categorical, "global", response, i, enumerations_dict
        )
    ...
    for i, curr_numerical in enumerate(numerical_variables):
        for j in range(num_datasets):
            numerical_model_list.append(
                numerical_to_result_model(curr_numerical, str(j), response, i)
            )
...
def numerical_to_result_model(name, dataset_id: str, response, column_id):
    if dataset_id == "global":
        ...
        data["q3"] = q3_dataset[dataset_id_numeric][column_id]
        dataset = dataset_name
    curr_summary = numerical_summary(
        num_datapoints=num_total,
        num_nulls=num_null,
        num_total=num_total - num_null,
        data=data,
...
def categorical_to_result(
    name, dataset_id: str, response, column_id, enumerations_dict
):
    if dataset_id == "global":
        ...
        dataset = dataset_name
    curr_summary = categorical_summary(
        num_datapoints=num_total,
        num_nulls=num_nulls,
...
def run(algo_interface):
    ...
    if categorical_columns:
        ...
        categorical_list_res = json.loads(global_categorical.get_table_data()[1][0])[
            "categorical_list"
        ]
    ...
    global_state, global_result = global_run(
        func=global_stats, positional_args=[local_result], share_to_locals=[False, True]
    )
    ...
    not_null_dataset = json.loads(global_result.get_table_data()[1][0])[
        "count_not_null_dataset"
    ]
    num_null_dataset = json.loads(global_result.get_table_data()[1][0])[
        "num_null_dataset"
    ]
    ...
    count_not_null_nn = json.loads(global_result_not_null.get_table_data()[1][0])[
        "count_not_null"
    ]
    count_num_null_nn = json.loads(global_result_not_null.get_table_data()[1][0])[
        "count_num_null"
    ]
    new_quartiles_nn = json.loads(global_result_not_null.get_table_data()[1][0])[
        "quartiles"
    ]

    maximum_dataset_nn = json.loads(global_result_not_null.get_table_data()[1][0])[
        "maximum_dataset"
    ]
    minimum_dataset_nn = json.loads(global_result_not_null.get_table_data()[1][0])[
        "minimum_dataset"
    ]
    std_dataset_nn = json.loads(global_result_not_null.get_table_data()[1][0])[
        "std_dataset"
    ]
    mean_dataset_nn = json.loads(global_result_not_null.get_table_data()[1][0])[
        "mean_dataset"
    ]

    not_null_dataset_nn = json.loads(global_result_not_null.get_table_data()[1][0])[
        "count_not_null_dataset"
    ]
    num_null_dataset_nn = json.loads(global_result_not_null.get_table_data()[1][0])[
        "num_null_dataset"
    ]
    ...
    new_obj = populate(
        result, numerical_columns, categorical_columns, enumerations_dict
    )
...
def global_stats(local_transfers):
    ...
    final_result = {
        ...
        "count_not_null": curr_count_not_null.tolist(),
        "count_num_null": curr_count_null.tolist(),
        "quartiles": quartiles_list,
        "maximum_dataset": maximum_list,
        "minimum_dataset": minimum_list,
        "std_dataset": std_list,
        "mean_dataset": mean_list,
        "num_null_dataset": num_null_list,
...
@udf(
    input_array=tensor(dtype=T, ndims=2),
