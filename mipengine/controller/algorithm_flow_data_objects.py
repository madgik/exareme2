...
class LocalNodesTable(LocalNodesData):
    ...
    @property
    def columns(self):
        # The columns of the table should not include the system internal "DATA_TABLE_PRIMARY_KEY"
        return [
            column.name
    ...
    # TODO Should be removed or disabled in PROD
...
class GlobalNodeTable(GlobalNodeData):
    ...
    @property
    def schema(self):
        return self._table_info.schema_

    @property
    def columns(self):
        return [
            (column.name, column.dtype) for column in self._table_info.schema_.columns
        ]
