...
class Connections:
    ...
    async def initialize(self):  ### create connection pools
        if self.db_objects["global"] == {}:
            ...
            #TODO:what you call dbname is a URL, this must be fixed
            #print(f"\n(Connections::initialize) self.glob->{self.glob}\n")
            self.db_objects["global"]["host"] = glob.path.replace("/","")
            #print(f"\n(Connections::initialize) self.db_objects->{self.db_objects}\n")
            ...
            for i, db in enumerate(servers.servers[1:]):
                ...
                local_node["host"] = loc.path.replace("/","")
...
#                    await local['async_con'].cursor().execute(udf.get("declaration"))
    ...
    async def acquire(self):  #### get connection objects
        ...
        db_conn["global"]["host"] = self.db_objects["global"]["host"]
        ...
        for db_object in self.db_objects["local"]:
            ...
            local_node["host"] = db_object["host"]
    ...
    #DEBUG
    def print_db_objects(self):
        print(f"\n(connection.py::Connections::print_db_objects): ")
        for item in self.db_objects.items():
            print(f"db_object->  {item}")
        #printself.db_objects->{self.db_objects}\n")
