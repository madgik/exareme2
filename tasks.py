...
def create_node_configs(c):
    """
    Create the node configuration files, using the '.deployment.toml' and store them in the `configs` folder.
    """
...
def create_monetdb(c, node, monetdb_image=None):
    ...
    If an image is not provided it will use `.deployment.toml`.

    :param node: A list of nodes for which it will create the monetdb containers.
    :param monetdb_image: The image to deploy. If not set, it will read it from the `.deployment.toml`.
    """
    if not node:
...
def load_data(c, port=None):
    """
    Load data into the specified DB from the integration_tests/data folder.

    :param port: A list of ports, in which it will load the data. If not set, it will use the `configs` folder files.
    """
...
def config_rabbitmq(c, node):
    ...
    :param node: A list of rabbitmq containers which it will configure.
    """
...
def create_rabbitmq(c, node):
    ...
    :param node: A list of rabbitmq containers which it will (re)create.
    """
    if not node:
        message("Please specify a node using --node <node>", Level.WARNING)
        sys.exit(1)
...
def start_node(c, node=None, all_=False, celery_log_level=None, detached=False):
    ...
    If the --detached argument is given, it will start the node service in the background.
    :param node: The node to start, using the proper file in the `configs` folder.
    :param all_: If set, the nodes of which the configuration file exists, will be started.
    :param celery_log_level: If not provided, it will look into the `.deployment.toml`.
...
def deploy(
    ...
    :param celery_log_level: Used for the engine services. If not provided, it will look into the `.deployment.toml`.
    :param monetdb_image: Used for the db containers. If not provided, it will look into the `.deployment.toml`.
    """
...
def start_flower(c, node=None, all_=False):
    ...
    :param all_: If set, it will create monitoring for all node services in the `configs` folder.
    """
