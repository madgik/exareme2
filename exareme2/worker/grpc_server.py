...
class WorkerService(worker_pb2_grpc.WorkerServiceServicer):
    ...
    def LoadDataFolder(self, request, context):
        folder = request.folder.value if request.HasField("folder") else None
        try:
            status = data_loader_service.load_data_folder(
                request_id=request.request_id, folder=folder
            )
            return worker_pb2.LoadDataFolderResponse(status=status)
        except Exception as exc:  # noqa: BLE001
            self._handle_exception(context, exc)
