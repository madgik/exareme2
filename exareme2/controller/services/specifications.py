...
from exareme2.algorithms.specifications import DeploymentType
...
class SpecificationLoader:
    ...
    def __init__(self):
        self._all_specs: Dict[str, SpecificationLoader.SpecType] = {}
        self.enabled_algorithms: Dict[str, AlgorithmSpecification] = {}
        self.enabled_transformers: Dict[str, TransformerSpecification] = {}
        self._flags = {
            DeploymentType.MONETDB: ctrl_config.monetdb.enabled,
            DeploymentType.AGGREGATION_SERVER: ctrl_config.aggregation_server.enabled,
        }
    ...
    def _filter_specifications(self) -> None:
        for spec in self._all_specs.values():
            ...
            if spec.deployment_types and (
                not any(self._flags.get(dt, False) for dt in spec.deployment_types)
            ):
                missing = (
                    spec.deployment_types[0].value
                    if spec.deployment_types
                    else "unspecified"
                )
                warnings.warn(
    ...
    def get_deployment_types(self, algo_name: str) -> List[DeploymentType]:
        if algo_name not in self.enabled_algorithms:
            raise KeyError(f"Algorithm '{algo_name}' not enabled or not found.")
        return self.enabled_algorithms[algo_name].deployment_types
