...
The purpose of these generators is to isolate the algorithm from the federated system so that the algorithm developer needs only standard Python and standard SQL and does not depends in any way on the system's internals and possible big changes that there may be on these internals.
